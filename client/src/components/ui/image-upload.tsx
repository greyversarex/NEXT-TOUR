...
export function ImageUpload({ value, onChange, placeholder = "https://...", className, onImageClick }: ImageUploadProps) {
    ...
    <div className={`flex gap-2 ${className}`}>
      ...
      {value && (
        isVideo(value) ? (
          ...
          <div
            className={`relative h-10 w-14 shrink-0 group ${onImageClick ? "cursor-pointer" : ""}`}
            onClick={onImageClick}
            title={onImageClick ? "Нажмите для настройки позиции" : undefined}
          >
            <img
              src={value}
              alt=""
            ...
            {onImageClick && (
              <div className="absolute inset-0 rounded bg-black/0 group-hover:bg-black/40 transition-colors flex items-center justify-center">
                <Settings2 className="h-4 w-4 text-white opacity-0 group-hover:opacity-100 transition-opacity" />
              </div>
            )}
          </div>
        )
